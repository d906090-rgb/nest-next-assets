...
export const LevelManager: React.FC = () => {
  ...
  useFrame((state, delta) => {
    ...
    // Get player position - player is always at z=0, only x changes based on lane
    if (playerObjRef.current) {
        ...
        if (group && group.children.length > 0) {
            ...
            // Fallback: player is at origin, z=0
            playerPos.set(0, 0, 0);
        }
    }
    // Player z is always 0 in this game (objects move toward player)
    playerPos.z = 0;
    ...
    for (const obj of currentObjects) {
        ...
        let keep = true;
        if (obj.active) {
            // Check if object passed through player zone (z around 0)
            const objZ = obj.position[2];
            const passedPlayer = prevZ <= 1.5 && objZ >= -1.5; // Object is near z=0
            ...
            if (obj.type === ObjectType.SHOP_PORTAL) {
                ...
                // Different collision radius for damage vs collectibles
                const collisionRadius = isDamageSource ? 0.8 : 1.3; // Tighter for obstacles, wider for pickups
