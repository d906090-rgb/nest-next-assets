...
// Shared Player Materials (Global reuse to prevent GPU memory pressure)
const MAT_ARMOR_BLUE = new THREE.MeshStandardMaterial({ color: '#00aaff', roughness: 0.3, metalness: 0.8 });
const MAT_ARMOR_GOLD = new THREE.MeshStandardMaterial({ color: '#ffd700', roughness: 0.3, metalness: 0.8 });
const MAT_JOINT = new THREE.MeshStandardMaterial({ color: '#111111', roughness: 0.7, metalness: 0.5 });
const MAT_GLOW_BLUE = new THREE.MeshBasicMaterial({ color: '#00ffff' });
const MAT_GLOW_WHITE = new THREE.MeshBasicMaterial({ color: '#ffffff' });
const MAT_PLAYER_SHADOW = new THREE.MeshBasicMaterial({ color: '#000000', opacity: 0.3, transparent: true });
...
export const Player: React.FC = () => {
  ...
  // Materials selection based on state
  const armorMaterial = isImmortalityActive ? MAT_ARMOR_GOLD : MAT_ARMOR_BLUE;
  const glowMaterial = isImmortalityActive ? MAT_GLOW_WHITE : MAT_GLOW_BLUE;
  const jointMaterial = MAT_JOINT;
  const shadowMaterial = MAT_PLAYER_SHADOW;
